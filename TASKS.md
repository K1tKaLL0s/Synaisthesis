...
- [x] M8.2 — 真实 Python 沙箱（Stage 8 第 2 个 Task）：integrations/docker/policy.py（DockerSandboxPolicy：network=none、无 host mounts、只读根、内存/CPU/pids 限制、超时、allowed_env 白名单、policy_hash）、verifiers/python/sandbox.py（真实 `docker run --rm -i` 执行、内容绑定 receipt（exit/stdout/stderr/image_id/elapsed/policy·code hash）、超时 `docker rm -f` 杀容器无孤儿、Docker 不可用 → TOOL_UNAVAILABLE）、tests/external/test_python_sandbox.py 真实 smoke（基础执行/断网/无 secret/无挂载/pids 限制/OOM 137/超时清理/回执）（7 个测试，全套 501 passed）

## Next

- [ ] M8.3.LEAN.COMPILER — 真实 Lean 适配器：真实 Lean 成功才写 E4、statement hash 变化退出 Proof Loop（前置 M8.2；Lean 4.32.2 工具链）

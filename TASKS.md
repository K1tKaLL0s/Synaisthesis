...
- [x] M9.1 — 真实 Council 纵切片（Stage 9 第 1 个 Task）：小 Claim 全链——Council run 启动（M6.2/M7.1 复用）→ 反例（确定性）→ repair 事件 → Semantic Gate（F4/F5 拒绝、仅真实用户 APPROVE）→ 真实 Lean 验证（exit 0 + receipt）→ 用户确认（绑定证据回执）→ Bundle 事件链（CouncilRunCreated/Started/RepairRecorded/SemanticGateResolved/EvidenceRecorded/SliceConfirmed）、regression 节点（statement hash 变化退出 Proof Loop）、council 验证/修复/语义/回归/确认节点（4 个测试，全套 512 passed）

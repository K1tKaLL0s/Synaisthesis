...
- [x] M2.3 — RQ 领域底座（Stage 2.5 第 1 个 Task）：RQ0–RQ4 不可变领域 Artifact；RQ2F 谓词保守聚合与固定真值表；理论 50+50 / 工程 60+40 两套评分 policy、逐项取小、69/70 固定路由；Gate 绑定与未通过 RQ 禁止转 S5/ENG0；19 个 RQ 事件名；migration 0002（22 个单元测试，全套 135 passed；验收通过，工作区未提交）

## Next

- [ ] M2.4.RQ.FAKE_RETRIEVAL — Fake 学术/工程近邻检索 Provider、去重/排序/coverage 计算（前置 M2.3 已验收 PASS；开始前需先处理 M2.3 提交）

...
## Active work unit
- Stable Task ID: `M13.1.ACADEMIC.PROVIDERS`
- Milestone: `M13`（Stage 13 第 1 个 Task：生产级学术近邻检索）
- WorkUnitContract: `workspace/workunit-contracts/M13.1.ACADEMIC.PROVIDERS.md`
- 交付：OpenAlex/Crossref/arXiv 适配器 + 稳定去重 + 归一化扩展；receipt 可追溯；失败结构化 Blocker。
...
- 代码方面：`M7.1.COUNCIL.STATE_GRAPH`（前置 M6.2 已 PASS；合同已备；有效轮/十轮/checkpoint/pause-resume/Gate 恢复）

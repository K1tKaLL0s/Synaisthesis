...
## Active work unit
- Stable Task ID: `M6.1.LLM.PROVIDER`
- Milestone: `M6`（Stage 6 第 1 个 Task：模型 Provider 与隔离）
- WorkUnitContract: `workspace/workunit-contracts/M6.1.LLM.PROVIDER.md`
- 交付：LLMProvider Protocol + FakeLLMProvider + 严格结构化输出 + UsageRecord；零新依赖。
...
- 代码方面：`M6.2.ROLE.ISOLATION`（前置 M6.1 已 PASS；必须读取 04 §3 与 08 §3/§7；三轨隔离与 Council 服务底座）
...
- M6.1 已提交：见 commit 记录（feat + chore(status) 两提交）。
